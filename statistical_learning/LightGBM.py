...
def get_lgb_data(x_train_sparse, y_train, x_test_sparse, y_test):
    d_train = lgb.Dataset(data=x_train_sparse, label=y_train)
    d_test = lgb.Dataset(data=x_test_sparse, label=y_test)
...
def train_model(d_train, d_test, params_lgb_2, verbose):
...
def train(x_train_sparse, y_train, x_test_sparse, y_test, n_estimators_0, objective, eval_metric, k, verbose):
    # get_lgb_data
    d_train, d_test = get_lgb_data(x_train_sparse, y_train, x_test_sparse, y_test)
    # define init params
    params_lgb_1 = LGBMRegressor(booster="gbdt",
                                 silent=True,
                                 n_jobs=-1,
                                 learning_rate=0.1,
    ...
    print("max_depth: {0}; min_child_weight: {1}".format(params_lgb_2["max_depth"], params_lgb_2["min_child_weight"]))
    ...
    params_lgb_1, params_lgb_2 = get_best_params(x_train_sparse, y_train, params_lgb_1, params_lgb_2, k,
                                                 params_hyp=dict(bagging_fraction=[0.6,0.7,0.8,0.9], 
    ...
    params_lgb_1, params_lgb_2 = get_best_estimators(d_train, params_lgb_1, params_lgb_2, k, verbose)
    # train_model
    model_lgb = train_model(d_train, d_test, params_lgb_2, verbose)
